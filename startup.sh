...
if grep -q 'server_host="0.0.0.0", server_port=args.port, allowed_paths=[output_dir])' "$REPO_HOME/gradio_demo.py"; then
    ...
    sed -i 's/server_port=args.port, allowed_paths=[output_dir])/server_host="0.0.0.0", server_port=args.port, allowed_paths=[output_dir])/g' "$REPO_HOME/gradio_demo.py"
